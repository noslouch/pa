...
<form action="<?=$action_url?>" method="POST">
	...
	<?php if(!empty($member_groups)):?>
		<?php $permissions = array('can_admin_own_profiles', 'can_view_group_profiles', 'can_admin_group_profiles', 'can_access_access_settings');?>
		...
		<?php foreach($member_groups as $member_group):?>
			...
			<tr class="<?=$row_class?>">
				...
				<td class="hoverable" style="cursor: auto">
					...
					<?php $class = 'group_id'; $name = 'members['.$member_group['group_id'].']['.$class.']';?>
					<?=form_hidden($name, $member_group['group_id'])?>
				</td>
				
				<?php foreach($permissions as $class) :?>
				<td class="hoverable <?=$clickable?>" style="cursor: auto">
					<?php 
						...
						$checked = ((isset($access_settings[$member_group['group_id']][$class]) && $access_settings[$member_group['group_id']][$class] == 'y') || $member_group['group_id'] == 1) ? TRUE : FALSE;
					?>
					<?=form_hidden($name, 'n')?>
					<?=form_checkbox($name, 'y', $checked, 'class="'.$toggle_class.'"'.$disabled)?>
		...
		<button type="submit" class="submit left withloader" tabindex="1000">
			...
			<span class="onsubmit invisible"><?=lang('saving')?> <i class="icon-spinner icon-spin icon-large"></i></span>
