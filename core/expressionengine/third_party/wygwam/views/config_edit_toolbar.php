		...
		<?php else: ?>
			<span id="<?php echo $id ?>" class="cke_toolbar tb-option <?php if ($selections_pane) echo 'tb-selected' ?>">
				...
				<?php if ($is_combo): ?>
					...
					<input type="hidden" name="settings[toolbar][]" value="<?php echo $button ?>" <?php if (!$selections_pane) echo 'disabled' ?>>
				<?php else: ?>
					<span class="cke_toolgroup">
						<?php foreach($group as $button): ?>
							<?php
								...
								$label = isset($vars['tb_label_overrides'][$button]) ? $vars['tb_label_overrides'][$button] : $button;
							?>
							<a class="cke_button cke_button_off cke_button__<?php echo $lc_class ?> <?php if ($disabled) echo 'disabled' ?>" title="<?php echo $label ?>">
